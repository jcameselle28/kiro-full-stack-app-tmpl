...
- **AWS CLI**: Pre-configured with account permissions
...
Impact: [potential impact explanation]
...
aws ecr describe-images --repository-name <repo-name> --query 'imageDetails | sort_by(@, &imagePushedAt) | [-10:].[imageTags[0],imagePushedAt]' --output table
